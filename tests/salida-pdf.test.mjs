...
test('la UI exporta agenda y ficha, captura errores y no exige GPS', async () => {
  ...
  assert.match(page, /Elegí el período de la agenda/)
  assert.match(page, /type="date"/)
  assert.match(page, /Descargar agenda PDF/)
  assert.match(page, /readAllRows<OutingRecord>/)
  assert.match(page, /meetingPointName: outing\.meeting_point_name\?\.trim\(\) \|\| null/)
  assert.doesNotMatch(page, /meetingPointName: textoPuntoSalida\(\{/)
